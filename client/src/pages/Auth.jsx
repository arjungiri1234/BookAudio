...
export default function Auth() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Strict validation to block fake-looking emails like "test12345"
    if (!email) return;
    ...
    // 2. Fake sequence/pattern blocking (e.g. test1234, 12345)
    const localPart = email.split('@')[0].toLowerCase();
    const isFakePattern =
      localPart.includes("test") ||
      /1234|qwerty|asdf/.test(localPart) ||
      (localPart.match(/\d/g) || []).length > 5; // Too many numbers

    if (isFakePattern && localPart.length > 3) {
        ...
        <div className="glass-card p-8">
          ...
          <div className="space-y-3 mb-6">
            <button
              ...
              className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-white border border-surface-300 rounded-xl text-surface-700 font-medium text-sm hover:bg-surface-50 hover:border-surface-400 transition-all duration-200 shadow-sm disabled:opacity-50"
            >
              {oauthLoading === "google" ? (
            ...
            <button
              ...
              disabled={!!oauthLoading}
              className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-surface-800 border border-surface-700 rounded-xl text-white font-medium text-sm hover:bg-surface-700 transition-all duration-200 shadow-sm disabled:opacity-50"
            >
              {oauthLoading === "github" ? (
                <span className="w-5 h-5 border-2 border-surface-500 border-t-white rounded-full animate-spin" />
              ) : (
                <GitHubIcon />
              )}
